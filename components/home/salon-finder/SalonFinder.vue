  ...
  <div class="salon-finder">
    <b-container>
      <div class="salon-box">
        ...
        <div class="text-white" v-html="$md.render(direccion)"></div>
      </div>
    </b-container>
...
export default {
  name: 'SalonFinder',
  props: ['direccion', 'mapurl'],
  data() {
    return {
      urlKmlayer:
        'https://www.google.com/maps/d/kml?mid=1_e8dWcmhAjBgsP_jhLEcUsxrB7Ffxfme',
  ...
  computed: {
    ...
    textsData() {
      return this.$store.state.siteConfigData.content_repeat.find(
        (element) => element.content_type == 'find a salon'
      )
    }
  },
  mounted() {
  ...
  methods: {
    ...
    showInContentWindow({ featureData, latLng }) {
      ...
      btnOpenInGMap.setAttribute(
        'href',
        `https://maps.google.com/?q=${latLng.lat()},${latLng.lng()}&z=8`
      )
